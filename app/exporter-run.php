...
if (isset($_GET['step'])) {
    ...
    if (($state['phase'] ?? 'discover') === 'discover') {
        ...
        if ($list === []) {
            $state['phase'] = 'export';
            $state['lastMessage'] = 'Discovery finished: no more rows returned.';
            save_state($stateFile, $state);
        ...
        if (isset($seenPageHashes[$pageHash])) {
            $state['phase'] = 'export';
            $state['discoveryStopReason'] = "Repeated page detected at offset {$offset}.";
            $state['lastMessage'] = 'Discovery stopped: repeated page detected. Total discovered ' . count($ids) . '.';
            save_state($stateFile, $state);
            echo json_encode(state_payload($state, $state['lastMessage']));
            exit;
        }
        ...
        if ($newAdded === 0) {
            $state['phase'] = 'export';
            $state['discoveryStopReason'] = "No new ids at offset {$offset}.";
            $state['lastMessage'] = 'Discovery stopped: page returned rows but no new reservation ids. Total discovered ' . count($ids) . '.';
            save_state($stateFile, $state);
            echo json_encode(state_payload($state, $state['lastMessage']));
            exit;
        }
        ...
        $state['lastMessage'] = 'Discovery page ' . $pagesScanned . ': new added ' . $newAdded . ', total discovered ' . count($ids) . '.';
        save_state($stateFile, $state);
...
header('Content-Type: application/json; charset=utf-8');
$state = load_state($stateFile);
echo json_encode(state_payload($state, (string)($state['lastMessage'] ?? 'Idle')));function save_state(string $stateFile, array $state): void
{
    file_put_contents($stateFile, json_encode($state, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
}
